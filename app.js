...
function computeBBox(){
  if(!E.cavity || !E.cavity.getBBox) return; const b = E.cavity.getBBox(); S.bbox = b;
  // Matcha fill & ytskugga till kavitetens bredd
  E.fill.setAttribute('x', b.x); E.fill.setAttribute('width', b.width);
  E.surfaceShade.setAttribute('x', b.x); E.surfaceShade.setAttribute('width', b.width);
}

function fillBottle(){
  const b = S.bbox; if(!b) return; const p = Math.max(0, Math.min(1, cur() / Math.max(1,S.target||1)));
  const h = Math.round(b.height * p); const y = b.y + (b.height - h);
  E.fill.setAttribute('y', y); E.fill.setAttribute('height', h);
  E.surfaceShade.setAttribute('y', y); // ytskuggan ligger överst på vätskan
...
// === Bubbler & nyckelpigor ===
let bubbleTimer, bugTimer; const BUG_COUNT = 8;
function spawnBubble(){ if(S.reduced) return; const b=S.bbox; if(!b) return; const cx = b.x + Math.random()*b.width; const r = 2 + Math.random()*4; const startY = b.y + b.height - 4; const life = 1800 + Math.random()*1800; const c = document.createElementNS('http://www.w3.org/2000/svg','circle'); c.setAttribute('cx', cx); c.setAttribute('cy', startY); c.setAttribute('r', r); c.setAttribute('fill','url(#wb-bubble)'); c.style.opacity='0.85'; E.bubbles.appendChild(c); const drift = (Math.random()*12 - 6);
  const t0 = performance.now(); function tick(t){ const dt = t - t0; const k = Math.min(1, dt/life); const y = startY - k*b.height*0.9; const x = cx + drift*Math.sin(k*Math.PI*2); c.setAttribute('cy', y); c.setAttribute('cx', x); c.style.opacity = String(0.85*(1-k)); if(dt < life){ requestAnimationFrame(tick); } else { c.remove(); } }
  requestAnimationFrame(tick);
}

const bugs = [];
function initBugs(){ const b=S.bbox; if(!b) return; E.bugs.innerHTML=''; bugs.length=0; for(let i=0;i<BUG_COUNT;i++){ const img=document.createElementNS('http://www.w3.org/2000/svg','image'); img.setAttributeNS('http://www.w3.org/1999/xlink','href','assets/ladybug.svg'); const x = b.x + 6 + Math.random()*(b.width-12); const y = b.y + 6 + Math.random()*(b.height*0.8);
    img.setAttribute('x', x); img.setAttribute('y', y); img.setAttribute('width', 20); img.setAttribute('height', 16); img.style.opacity='0.9'; E.bugs.appendChild(img);
    const dir = Math.random()<0.5? -1: 1; const amp = 6+Math.random()*10; const speed = 4000 + Math.random()*5000; const baseRot = Math.random()*20-10; bugs.push({img,x,y,dir,amp,speed,baseRot,t0:performance.now()+Math.random()*1000}); }
}
function animateBugs(){ if(S.reduced) return; const b=S.bbox; if(!b) return; function step(t){ for(const bug of bugs){ const k = ((t - bug.t0) % bug.speed)/bug.speed; const yy = bug.y + Math.sin(k*2*Math.PI)*bug.amp; const xx = bug.x + Math.cos(k*2*Math.PI)*bug.amp*0.6*bug.dir; const rot = bug.baseRot + Math.sin(k*2*Math.PI)*6*bug.dir; bug.img.setAttribute('x', xx); bug.img.setAttribute('y', yy); bug.img.setAttribute('transform',`rotate(${rot} ${xx+10} ${yy+8})`); } requestAnimationFrame(step);} requestAnimationFrame(step); }
...
(async function init(){ const prefers=window.matchMedia('(prefers-reduced-motion: reduce)').matches; S.reduced=prefers; if(E.motion) E.motion.checked=prefers; applyMotion(); computeBBox(); await loadFromSheet(); computeBBox(); renderAll(); startFluidFX(); })();
